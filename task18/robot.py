...
Вижу так, что каждая операция возвращает доступный набор следующих операций в виде lambda-функций,
либо в виде еще какого-то контекста 

При этом получается, что набор доступных операций зависит от того, что же именно вернули в ходе конкретного вычисления

но в любом случае 

Можем создать некоторый объект 
Operation - с возможностью  сравнивать.
Список разрешенных операций можно хранить так же в состоянии. Либо передавать в state monad
Либо передавать вместе callable объектом в операции bind, делая это как-то неявно.  

Но по сути список разрешенных операций связан с вызовом call - т.е. мы в любом случае получаем его динамически. 

мы например делаем .bind(move(150)).bind(move(150))
Но при этом мы как-то на основе заданного нам набора действий должны вообще фильтровать то, что можем
А как это делать? Потому что мы в момент 

move (150) >> move(150) 

Operation - сравниваем доступные операции
'''
